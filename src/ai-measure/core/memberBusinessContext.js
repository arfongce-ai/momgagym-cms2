...
// [매출 데이터 연결 배선 완료 2026-08-08] 관리자용 비즈니스 인사이트
// (functions/api/momi.js의 ADMIN_ROLE_SUFFIX)가 실제로 참고할 데이터가 이제
// 있다. 이 파일은 회원 객체에서 결제·출석·이용권 관련 "신호"만 안전하게
// 요약해서 뽑아낸다. 전화번호·생년월일·메모 같은 개인정보나 원본 결제 금액은
// 절대 포함하지 않는다 — 재등록·이탈 판단에 필요한 신호만.
//
// [배선 완료 확인 2026-08-09] momiService.js의 askMomi()가 매 호출마다 이 함수를
// 불러 businessContext로 서버에 함께 보낸다(리포트 컴포넌트마다 따로 붙일 필요
// 없음 — MomiInsightPanel을 쓰는 7개 리포트 화면 전부에 이미 자동 적용됨).
