...
//
//  ※ 이 파일은 "화면에 뭘 보여줄지"만 계산한다. [2026-08-06] 이 중 방향·크기가
//  명확한 5개(CoG류 2개·머리기울기·팔꿈치폄·팔꿈치비대칭)는
//  squatBiomechanics.js/squatBiomechanicsTracker.js에 별도로 연결되어 이제
//  정상/주의/위험 판정에 반영된다(연결 로직은 그쪽 파일에 있고, 계산 공식은
//  이 파일과 같은 패턴이지만 별도 구현 — 자세한 이유는 그 파일 주석 참고).
//  나머지 6개(관절별 굽힘 4개·귀-어깨 간격 2개)는 스쿼트 깊이에 따라 계속
//  바뀌는 각도/카메라 거리에 흔들리는 값이라 단일 목표값을 정할 수 없어
//  여전히 순수 표시만 한다.
