...
export default function RsiMeasure({ member, onSave, onBack }) {
  ...
  const grade = result
    ? result.rsi >= 2.5 ? { label: '매우 우수', color: 'text-blue-700 dark:text-blue-400' }
    : result.rsi >= 2.0 ? { label: '우수', color: 'text-emerald-700 dark:text-emerald-400' }
    : result.rsi >= 1.5 ? { label: '보통', color: 'text-amber-700 dark:text-amber-400' }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-3">
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">접지 시간 (초)</label>
      ...
      {result && (
        <div className="card-accent p-4 space-y-3 animate-fade-in">
          <div className="flex items-baseline justify-between">
            <p className="text-xs font-bold text-amber-700 dark:text-amber-400 uppercase tracking-widest">RSI</p>
            <p className={`text-sm font-bold ${grade.color}`}>{grade.label}</p>
          </div>
          <p className="text-center font-mono font-black text-5xl text-slate-800 dark:text-slate-100">{result.rsi}<span className="text-lg text-slate-500"> (체공/접지)</span></p>
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="bg-slate-100 dark:bg-slate-800 rounded-xl py-2">
              <p className="text-[10px] text-slate-500">점프 높이</p>
              <p className="font-mono font-bold text-slate-700 dark:text-slate-200">{result.heightCm} cm</p>
            </div>
            <div className="bg-slate-100 dark:bg-slate-800 rounded-xl py-2">
              <p className="text-[10px] text-slate-500">RSI(높이)</p>
              <p className="font-mono font-bold text-slate-700 dark:text-slate-200">{result.rsiHeight} m/s</p>
            </div>
            <div className="bg-slate-100 dark:bg-slate-800 rounded-xl py-2">
              <p className="text-[10px] text-slate-500">이륙 속도</p>
              <p className="font-mono font-bold text-slate-700 dark:text-slate-200">{result.takeoffVelocity} m/s</p>
            </div>
          </div>
