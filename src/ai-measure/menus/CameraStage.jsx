...
export default function CameraStage({
  ...
  return (
    <div className="cam-stage">
      ...
      {/* 로딩/오류 안내 */}
      {status !== 'running' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-300 text-sm text-center px-6 gap-3">
          {status === 'loading' && (
            <>
      ...
      {/* 상단 닫기 + 가이드 */}
      <div className="absolute top-0 left-0 right-0 pt-[max(env(safe-area-inset-top),12px)] px-3 pb-2 bg-gradient-to-b from-black/70 to-transparent">
      ...
      {/* 하단 컨트롤 + (선택)결과 시트 */}
      <div className="absolute bottom-0 left-0 right-0 pb-[max(env(safe-area-inset-bottom),16px)] pt-3 px-3 bg-gradient-to-t from-black/80 via-black/55 to-transparent space-y-3">
        {children}
