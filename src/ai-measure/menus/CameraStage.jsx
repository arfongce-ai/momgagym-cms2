...
//   topOffset           : 상위(허브)가 이미 화면 상단에 겹쳐 그리는 오버레이의
//                         실측 높이(px). 역도/VBT/1RM 허브처럼 모드·종목 선택
//                         바를 화면 상단에 별도로 띄우는 임베드 모드에서, 이
//                         스테이지 자체의 "✕닫기·안내칩" 줄이 그 바와 겹치지
//                         않도록 그만큼 아래로 밀어낸다(오버레이 겹침 버그 수정).
...
export default function CameraStage({
  ...
  const topPad = `calc(env(safe-area-inset-top) + ${10 + Math.max(0, topOffset)}px)`;
  const recTop = `calc(max(env(safe-area-inset-top), 12px) + ${Math.max(0, topOffset)}px)`;
  const seedHintTop = `calc(${Math.max(0, topOffset)}px + 34%)`;
  ...
  return (
    <div className="cam-stage">
      ...
      {recording && status === 'running' && (
        <div className="absolute left-1/2 z-30 -translate-x-1/2 rounded-full bg-red-500/80 border border-white/20 px-3 py-1.5 text-xs font-black text-white shadow-lg backdrop-blur"
          style={{ top: recTop }}>
      ...
      {showSeedHint && status === 'running' && countdown == null && (
        <div className="pointer-events-none absolute left-1/2 z-30 w-[min(88vw,360px)] -translate-x-1/2 rounded-2xl border border-amber-400/45 bg-black/70 px-4 py-3 text-center shadow-xl backdrop-blur animate-fade-in"
          style={{ top: seedHintTop }}>
      ...
      {/* 상단 닫기 + 가이드 */}
      <div className="absolute top-0 left-0 right-0 z-20 px-3 pb-8 bg-gradient-to-b from-black/55 via-black/25 to-transparent"
