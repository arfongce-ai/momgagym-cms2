...
export default function CameraStage({
  ...
  return (
    <div className="cam-stage">
      ...
      {aspectFrame && status === 'running' && (
        <div className="pointer-events-none absolute inset-0 z-[15] flex items-center justify-center p-2">
          <div className="relative border-2 border-amber-400/50 rounded-sm"
            ...
            <span className="absolute top-1 right-1 rounded bg-black/55 px-1.5 py-0.5 text-[9px] font-black text-amber-700 dark:text-amber-300">
      ...
      {/* 로딩/오류 안내 */}
      {status !== 'running' && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-slate-600 dark:text-slate-300 text-sm text-center px-6 gap-3">
          {status === 'loading' && (
            <>
          ...
          {status === 'error' && (
            <>
              <p className="text-red-700 dark:text-red-400 font-bold">카메라를 열 수 없습니다</p>
              <p className="text-xs text-slate-500 dark:text-slate-400 break-keep">{error || '권한을 허용했는지 확인하세요.'}</p>
            </>
          )}
        </div>
      )}
      ...
      {showSeedHint && status === 'running' && countdown == null && (
        <div className="pointer-events-none absolute left-1/2 z-30 w-[min(88vw,360px)] max-w-[calc(100vw-24px)] -translate-x-1/2 rounded-2xl border border-amber-400/45 bg-black/70 px-4 py-3 text-center shadow-xl backdrop-blur animate-fade-in break-keep" style={{ top: seedHintTop }}>
          <p className="text-sm font-black text-amber-700 dark:text-amber-300 break-keep">바벨 끝·원판 추적점을 먼저 1개 이상 눌러주세요</p>
          <p className="mt-1 text-[11px] font-bold text-slate-600 dark:text-slate-300 break-keep">2~3개 지정하면 가려져도 더 안정적입니다.</p>
        </div>
      )}
