...
export default function JumpMeasure({ member, onSave, onBack }) {
  ...
  const grade = result
    ? result.heightCm >= 50 ? { label: '매우 우수', color: 'text-blue-700 dark:text-blue-400' }
    : result.heightCm >= 40 ? { label: '우수', color: 'text-emerald-700 dark:text-emerald-400' }
    : result.heightCm >= 30 ? { label: '보통', color: 'text-amber-700 dark:text-amber-400' }
    ...
    <div className="space-y-4">
      ...
      {result && (
        <div className="card-accent p-4 space-y-3 animate-fade-in">
          ...
          <p className="text-center font-mono font-black text-5xl text-slate-800 dark:text-slate-100">{result.heightCm}<span className="text-lg text-slate-500"> cm</span></p>
          <div className="grid grid-cols-2 gap-2 text-center">
            ...
            <div className="bg-slate-100 dark:bg-slate-800 rounded-xl py-2">
              ...
              <p className="font-mono font-bold text-slate-700 dark:text-slate-200">{result.peakPower != null ? result.peakPower + ' W' : '체중 입력'}</p>
            </div>
          </div>
