...
export default function HeightField({ value, onChange, member, hint = 'cm 환산에 사용' }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3 space-y-2">
      <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">키 (cm)</label>
      <div className="flex items-center gap-2">
        <input
          type="number" inputMode="numeric" value={input}
          onChange={e => setInput(e.target.value)}
          placeholder="예: 170"
          className="flex-1 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-amber-500"
        />
      ...
      <div className="flex items-center gap-2 min-h-[16px]">
        {applied && <span className="text-[11px] text-emerald-700 dark:text-emerald-400 font-bold">✓ {value}cm 적용됨</span>}
        {fromRecord && <span className="text-[10px] text-emerald-700 dark:text-emerald-400/80">회원 기록에서 불러옴</span>}
