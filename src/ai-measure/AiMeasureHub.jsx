...
export default function AiMeasureHub() {
  ...
  // [가로모드 메뉴 예외 2026-09-07] registry.js에 orientation:'landscape'로 표시된
  // 메뉴(현재 스프린트)가 열려있는 동안은 전역 세로고정을 끈다 — 트랙 전체를 담으려면
  // 가로가 맞고, 세로 고정 훅이 켜진 채로는 SprintLiveAnalysis.jsx 자체의 "가로로
  // 돌려주세요" 안내와 정반대로 충돌해(자동회전 꺼진 기기에서 세로고정 오버레이가
  // 덮어써버림) 가로 화면을 못 쓰게 된다. id를 하드코딩하지 않고 registry 필드를
  // 보는 이유는 앞으로 가로가 필요한 측정이 늘어도 이 줄을 안 고치기 위해서다.
  // 나머지(보행/점프/자세/ROM 등)는 원래대로 세로 기준 레이아웃이라 계속 세로 고정.
  const isPortraitBlocked = useLockPortrait(active?.orientation !== 'landscape');
  ...
  // [모미 신규] "모미야 OO님 점프 측정하게 해줘" 같은 음성 명령으로 이 화면에 도착했으면,
  // 지정된 회원과 측정 종류를 자동으로 선택해준다. 1회성(consumePendingVoiceTarget이 즉시 지움)이라
  // 새로고침하거나 다시 들어와도 재실행되지 않는다.
  useEffect(() => {
    const pending = consumePendingVoiceTarget();
    if (!pending) return;
    if (pending.memberName) {
      const matched = members.find((m) => m.name === pending.memberName);
      if (matched) handleSelectMember(matched.id);
    }
    if (pending.testId) {
      const menu = MEASURE_MENUS.find((m) => m.id === pending.testId && m.status === 'ready');
      if (menu) openMenu(menu);
    }
    // 마운트 시 1회만 — 의도적으로 의존성 배열을 비워둠.
