...
export const UNIFIED_REPORT_PAGE_CLASS =
  'report-a4-page w-full max-w-[794px] rounded-2xl bg-white dark:bg-slate-900 p-5 text-slate-800 dark:text-slate-100 shadow-2xl ring-1 ring-slate-700/70 sm:p-6';

export function UnifiedReportCanvas({ children, className = '' }) {
  return (
    <div className={`min-h-full w-full bg-slate-50 dark:bg-slate-950 p-4 text-slate-800 dark:text-slate-100 ${className}`}>
      {children}
    </div>
  );
}
...
export function UnifiedReportFooter() {
  return (
    <footer className="mt-8 flex items-center justify-center border-t border-slate-200 dark:border-slate-800 pt-4">
...
export function UnifiedReportHeader({
  ...
  return (
    <header className={`border-b border-slate-300/70 dark:border-slate-700/70 ${compact ? 'pb-4 mb-5' : 'pb-5 mb-6'}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <p className="break-keep text-[11px] font-black uppercase tracking-[0.18em] text-amber-700 dark:text-amber-400 sm:text-[12px] sm:tracking-[0.22em]">
              {eyebrow}
            </p>
            {badge && (
              <span className="rounded-full border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-[9px] font-black text-slate-600 dark:text-slate-300">
                {badge}
              </span>
            )}
          </div>
        ...
        <div className="flex shrink-0 items-start gap-3">
          ...
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-100/80 dark:bg-slate-800/80 px-3 py-2 text-sm font-bold text-slate-600 dark:text-slate-300 hover:border-slate-500 hover:text-white"
            >
              닫기
            </button>
          )}
...
function BodyInfoStrip({ member }) {
  ...
  return (
    <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 rounded-lg bg-slate-100/50 dark:bg-slate-800/50 px-3 py-2 text-[11px] font-bold">
      <span className="text-slate-500">신체정보</span>
      <span className="font-mono text-slate-700 dark:text-slate-200">{parts.join(' · ')}</span>
      {snapshot.date && (
...
export function UnifiedReportSection({ title, subtitle, children, className = '' }) {
  return (
    <section className={`rounded-xl border border-slate-300/70 dark:border-slate-700/70 bg-slate-100/35 dark:bg-slate-800/35 p-4 ${className}`}>
      {(title || subtitle) && (
...
export function UnifiedEmptyState({ children = '리포트 데이터가 없습니다.', onClose }) {
  return (
    <UnifiedReportCanvas>
      <div className="mx-auto flex min-h-[320px] w-full max-w-[794px] flex-col items-center justify-center rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 text-center">
        <p className="text-sm font-bold text-slate-500 dark:text-slate-400">{children}</p>
        {onClose && (
          <button type="button" onClick={onClose} className="mt-4 rounded-lg bg-slate-100 dark:bg-slate-800 px-4 py-2 text-sm font-bold text-slate-700 dark:text-slate-200">
            닫기
          </button>
        )}
      </div>
...
export function ScoreRing({ score, label = '종합 점수' }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      >
        <div className="grid h-14 w-14 place-items-center rounded-full bg-white dark:bg-slate-900 text-center">
...
export function MetricCard({ metric }) {
  ...
  return (
    <div className="min-w-0 rounded-xl bg-slate-100/70 dark:bg-slate-800/70 px-3 py-3 ring-1 ring-slate-700/60">
      <div className="flex items-start justify-between gap-2">
        <p className="break-keep text-[11px] font-black leading-tight text-slate-600 dark:text-slate-300">{metric?.label || '측정 항목'}</p>
        <TrafficLightBadge status={token} />
      </div>
...
export function UnifiedReportCard({ title, subtitle, score, metrics = [], children }) {
  return (
    <section className="w-full max-w-[430px] rounded-2xl bg-white dark:bg-slate-900 p-5 text-slate-800 dark:text-slate-100 shadow-2xl ring-1 ring-slate-700/70 sm:max-w-[794px]">
      <header className="flex items-start justify-between gap-4 border-b border-slate-300/70 dark:border-slate-700/70 pb-4">
        <div className="min-w-0">
