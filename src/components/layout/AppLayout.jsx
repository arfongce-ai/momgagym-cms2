    ...
    <div className={`flex h-screen overflow-hidden ${bg} ${txt}`}>
        ...
        <div className={`mt-auto pt-4 border-t ${darkMode?'border-slate-800':'border-slate-200'}`}>
          ...
          {kioskOn ? (
            <button onClick={() => setUnlockOpen(true)}
              className="w-full text-left text-xs px-3 py-2 mb-1 rounded-lg text-amber-400 hover:bg-slate-800 transition-colors">
              🔒 키오스크 모드 해제
            </button>
          ) : (
            <button onClick={enableKiosk}
              className={`w-full text-left text-xs px-3 py-2 mb-1 rounded-lg ${sub} hover:text-amber-400 hover:bg-slate-800 transition-colors`}>
              키오스크 모드 켜기
            </button>
          )}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className={`md:hidden flex items-center justify-between px-4 py-3 border-b ${side}`}>
          ...
          <div className="flex items-center gap-2">
            {kioskOn ? (
              <button onClick={() => setUnlockOpen(true)} className="text-[11px] font-bold text-amber-400">🔒 해제</button>
            ) : (
