 ...
 */
export function setPendingVoiceTarget({ memberName = null, testId = null, openReportKind = null } = {}) {
  if (!memberName && !testId && !openReportKind) return;
  try {
    sessionStorage.setItem(STORAGE_KEY, JSON.stringify({ memberName, testId, openReportKind, ts: Date.now() }));
  } catch (e) {
